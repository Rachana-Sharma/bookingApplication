...
public interface RoomRepository extends CrudRepository<RoomEntity, Integer> {
	 ...
	 * @return room id
	 */
	@Query(value = "SELECT TOP 1 ROOM_ENTITY.ROOM_ID FROM ROOM_ENTITY WHERE "
			+ "(ROOM_ENTITY.ROOM_TYPE=:roomType AND ROOM_ENTITY.ROOM_ID IN (SELECT BOOKING_ENTITY.ROOM_ID FROM BOOKING_ENTITY WHERE BOOKING_ENTITY.END_DATE <:=startDate "
			+ "OR BOOKING_ENTITY.START_DATE >=:endDate))"
